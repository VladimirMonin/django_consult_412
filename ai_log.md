...
## Консультация по командам Django: dumpdata и loaddata

**Вопрос:** Какая команда загрузки данных соответствует команде `python -Xutf8 manage.py dumpdata -o data.json`?

**Ответ:** Для загрузки данных используется команда `python -Xutf8 manage.py loaddata data.json`

**Пояснение:**

- `dumpdata` выгружает данные из БД в JSON
- `loaddata` загружает данные из JSON в БД
- Параметр `-Xutf8` важен для корректной работы с кириллицей
- При конфликтах ID можно использовать флаг `--ignore`

P.S. Как говорится: что выгрузили - то и загружаем! Django знает как работать с парными командами.
